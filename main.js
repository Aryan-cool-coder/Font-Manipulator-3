...
sketch = "";
timer_counter = 0;
timer_check = new String("");
drawn_sketch = new String("");
answer_holder = new String("");
score = 0;

    random_no = Math.floor((Math.random()*quick_draw_data_set.length)+1);
    sketch = quick_draw_data_set[random_no];
    console.log(sketch);
    document.getElementById("ADText").innerHTML = quick_draw_data_set[random_no];
...
function gotResult(error, results) {
    if (error) {
        ...
        document.getElementById("EDText").innerHTML = drawn_sketch;
        document.getElementById("confidence").innerHTML = "confidence: " + Math.round(results[0].confidence*100) + "%";
    
}

function modelLoaded() {
    console.log("Doodle net model has been initialized");
}